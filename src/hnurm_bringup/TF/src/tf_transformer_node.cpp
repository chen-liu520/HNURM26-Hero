...
namespace hnurm
{
    ...
    void TfTransformer::init_and_static_transform()
    {
        ...
        // 1. 静态变换  实时点云坐标系：雷达开机位置/cloud_registration frame_id -> odom：完全重合
        ...
        // 2. 静态变换  base_link -> basefootprint：外参传入
        ...
        // 3. 静态变换  joint_link -> gunpoint：外参传入
    ...
    void TfTransformer::timer_callback()
    {
        /*tf tree
                                                                    ...
                                                                    lookup  --> gunpoint_horizontal
                                                                            |
           map  -->      odom     --> base_link  -->  base_link  -->  base_footprint --> joint_link --> gunpoint_link
                 |                 |               |               |                   |               |
                 |                 |               |               |                   |               |
              static           FAST-LIVO2         static        static              rexv_pitch       static
        */
        /********************************************************************/
        /*******************************静态变换*******************************/
        /********************************************************************/
        if (!is_static_finished_)
        {
    ...
    {
        if (!is_static_finished_ || !is_odom_ready_)
        {
            return;
        }
