...
private:
    void publishStaticMapToOdom()
    {
        ...
        // 固定平移，用于区分map和odom
        static_tf.transform.translation.x = 1.0;  // map原点在odom前方1米
        static_tf.transform.translation.y = 2.0;  // map原点在odom左侧2米
        ...
        RCLCPP_INFO(get_logger(), "Published static TF: map -> odom (translation: 1.0, 2.0, 0.0)");
    }
    ...
    void timerCallback()
    {
        ...
        // 设置RPY角度（只有pitch可变）
        recv_msg.roll = 0.0;
        recv_msg.pitch = current_pitch_;   // 动态变化的pitch
        recv_msg.yaw = 0.0;
        ...
        // 3. 发布 Odometry: odom -> base_link
        // 原始是 camera_init -> aft_mapped，这里映射为 odom -> base_link
        nav_msgs::msg::Odometry odom_msg;
        odom_msg.header.stamp = now;
        odom_msg.header.frame_id = "camera_init";  // 原始父坐标系
        odom_msg.child_frame_id = "aft_mapped";    // 原始子坐标系
        
        // 固定位置，稍微远一点用于区分
        odom_msg.pose.pose.position.x = 0.5;  // 在odom前方0.5米
        odom_msg.pose.pose.position.y = 0.3;  // 在odom左侧0.3米
        odom_msg.pose.pose.position.z = 0.1;  // 稍微抬高
